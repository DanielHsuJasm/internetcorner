document.addEventListener('DOMContentLoaded', function() {
    ...
    const letterContent = envelopeContainer ? envelopeContainer.querySelector('.letter-content') : null;
    ...
    function showImageOverlay(src, alt) {
        ...
        if (envelopeContainer) envelopeContainer.style.visibility = 'hidden';
    ...
    function hideImageOverlay() {
        ...
        setTimeout(() => {
            ...
            if (envelopeContainer) envelopeContainer.style.visibility = '';
            document.body.focus();
        }, 300);
    }
    ...
    /* ---------- 信封從右下小到全螢幕動畫 & tooltip ---------- */
    function initEnvelopeBehavior() {
        if (!envelopeContainer || !letterContent) return;
        let isFullscreen = false;

        envelopeContainer.addEventListener('click', toggleEnvelopeFullscreen);
        envelopeContainer.addEventListener('keydown', function(e) {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                toggleEnvelopeFullscreen();
            }
        });
        document.addEventListener('keydown', function(e) {
        ...
        function toggleEnvelopeFullscreen() {
            if (!isFullscreen) {
                enterFullscreen();
            } else {
                exitFullscreen();
        ...
        function enterFullscreen() {
            ...
            document.body.style.overflow = 'hidden';
            envelopeContainer.classList.add('fullscreen');
            // 等待 transition 完成再顯示內容
            const dur = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--envelope-anim-duration')) * 1000;
            setTimeout(() => {
        ...
        function exitFullscreen() {
            ...
            document.body.style.overflow = '';
            envelopeContainer.classList.remove('fullscreen');
        }
    }
